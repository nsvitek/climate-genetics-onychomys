...
# phenotypic variables are 
phenotype<-PCA.phenotype$x[,c(1:38)]
# genetic variables are
genotype<-PCA.genotype$li[,c(1:58)]

sample.populations<-metadata$`Sampling Area`
sample.populations<-metadata$`Genetic Group`
# sample.populations<-metadata$`Genetic Group`=="WB" #for fun, but doesn't make a difference

Fst(genotype,sample.populations,unique(sample.populations))

#Pst variables
h2.levels<-seq(from=0.1, to=0.9, by=0.1)
h.squared<-h2.levels[1] #initial estimate to model PDP's Pst
add.gen.proportion<-0.25 #c, assumed additive genetic proportion of differences between populations

# print("Observed Pst")
Pst(phenotype,add.gen.proportion,h.squared,sample.populations,unique(sample.populations))

for (i in 1:length(h2.levels)){
  Pst(phenotype,add.gen.proportion,h2.levels[i],sample.populations,unique(sample.populations)) %>% print
}

#confidence intervals by jackknifing samples
Fst.CI<-NULL
Pst.CI<-NULL
for (i in 1:nrow(genotype)){
  gen.CI<-genotype[-i,]
  sample.populations.CI<-sample.populations[-i]
  Fst.CI[i]<-Fst(gen.CI,sample.populations.CI,unique(sample.populations.CI))
  phen.CI<-phenotype[-i,]
  Pst.CI[i]<-Pst(phen.CI,add.gen.proportion,h.squared,sample.populations.CI,unique(sample.populations.CI))
